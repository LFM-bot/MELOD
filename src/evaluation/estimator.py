...
def load_estimator(config):
    if config.SR_task == 'Next-Session':
        return NextSessionEstimator(config)
...
class Estimator:
    ...
    def neg_sample_select(self, data_dict, prediction):
        ...
        if self.eval_mode == 'uni':
            ...
            sample_prob = self.popularity.unsqueeze(0).expand(*prediction.shape)
        else:
...
class NextSessionEstimator(Estimator):
    def __init__(self, config):
        super(NextSessionEstimator, self).__init__(config)
    ...
    def _reset_metrics(self):
        for metric in self.metrics:
            for k in self.k_list:
                self.metric_res_dict[f'{metric}@{k}'] = 0.
                for step in range(self.session_len):
                    self.metric_res_dict[f'step{step}-{metric}@{k}'] = 0.
        self.eval_loss = 0.

    @torch.no_grad()
    def evaluate(self, eval_loader, model):
        eval_loss = 0.
        metric_res_dict = self.test(eval_loader, model)

        return metric_res_dict, 0.
    ...
    def test(self, test_loader, model):
        ...
        test_sample_size = len(test_loader.dataset)
        test_iter = tqdm(enumerate(test_loader), total=len(test_loader))
        test_iter.set_description(f'do evaluation...')
        for _, batch_dict in test_iter:
            batch_to_device(batch_dict, self.dev)
            logits = model(batch_dict)
            logits = self.neg_sample_select(batch_dict, logits)
            batch_session_target = batch_dict['target']
            for step in range(self.session_len):
                target = batch_session_target[:, step]
                self.calc_metrics(logits, target, step)

        for metric in self.metrics:
            for k in self.k_list:
                total_score = 0.
                for step in range(self.session_len):
                    key = f'step{step}-{metric}@{k}'
                    total_score += self.metric_res_dict[key]
                    self.metric_res_dict.pop(key)
                self.metric_res_dict[f'{metric}@{k}'] = total_score / float(test_sample_size) / self.session_len

        return self.metric_res_dict

    def calc_metrics(self, prediction, target, step=0):
        _, topk_index = torch.topk(prediction, self.max_k, -1)  # [batch, max_k]
        topk_socre = torch.gather(prediction, index=topk_index, dim=-1)
        idx_sorted = torch.argsort(topk_socre, dim=-1, descending=True)
        top_k_item_sorted = torch.gather(topk_index, index=idx_sorted, dim=-1)
        ...
        for metric in self.metrics:
            for k in self.k_list:
                score = getattr(Metric, f'{metric.upper()}')(top_k_item_sorted, target, k)
                self.metric_res_dict[f'step{step}-{metric}@{k}'] += score
