...
class DataProcessor:
    ...
    def _train_test_split(self, seq_data_list):
        if self.split_type == 'valid_only':
            if self.SR_task == 'Next-Session':
                train_x, train_y, eval_x, eval_y = self._leave_one_out_split_for_next_session(seq_data_list)
            else:
    ...
    def _leave_one_out_split_for_next_session(self, seq_data):
        train_x = [item_seq[:-self.session_len - 1] for item_seq in seq_data if len(item_seq) > self.session_len + 1]
        train_y = [item_seq[-self.session_len - 1] for item_seq in seq_data if len(item_seq) > self.session_len + 1]
        eval_x = [item_seq[:-self.session_len] for item_seq in seq_data if len(item_seq) > self.session_len]
        eval_y = [item_seq[-self.session_len:] for item_seq in seq_data if len(item_seq) > self.session_len]
        return train_x, train_y, eval_x, eval_y

    def prepare_kg_map(self):
        kg_path = os.path.join(self.data_path, f'{self.dataset}_kg.npy')
        # kg_path = os.path.join(self.data_path, f'{self.dataset}_transe.npy')
        # kg_path = os.path.join(self.data_path, f'{self.dataset}_ssl.npy')
...
if __name__ == '__main__':
    train_x = [[2, 3]]
    item_seq = [1]
    train_x.append(item_seq)
    aug_seq_x = [item_seq[:last] for item_seq in train_x for last in range(1, len(item_seq))]
    print(aug_seq_x)
